...
const paymentMethod = ref<'card' | 'pix' | 'boleto'>('card');
...
async function checkoutCart() {
  ...
  loading.value = true;
  try {
    const checkout = await api<{ data: { checkout_url?: string; mode?: string } }>('/api/v1/orders/checkout', { method: 'POST', headers: { 'Idempotency-Key': `cart-${Date.now()}-${Math.random().toString(36).slice(2)}` }, body: JSON.stringify({ tickets: cart.value.map(ticket => ({ game_id: ticket.game.id, draw_id: ticket.draw_id, numbers: ticket.numbers, pool_id: ticket.pool_id, shares: ticket.shares })), method: paymentMethod.value }) });
...
<template>
  <div class="app-shell">
    ...
    <main v-if="view === 'home'" class="main">
      <section class="hero">
        <div class="hero-copy"><div class="eyebrow">Seu próximo momento pode começar aqui</div><h1>Jogue com leveza. Sonhe grande.</h1><p>Escolha seus números, participe de bolões inteligentes e acompanhe tudo em um só lugar — com transparência em cada etapa.</p><button class="btn btn-yellow" @click="navigate('games')">Escolher meu jogo <ArrowRight :size="16" /></button></div>
