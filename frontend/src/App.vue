...
type Game = { id: number; slug: string; name: string; short_name: string; price_cents: number; price_table?: Record<string, number>; official_price_table?: Record<string, number>; color: string; range_max: number; number_min?: number; numbers_required: number; min_numbers?: number; max_numbers?: number; selection_mode?: string; special_options?: { columns?: number; special_type?: string }; rules_source_url?: string | null; next_draw?: { id?: number; contest_number: number; draw_at: string; sales_close_at?: string | null }; latest_result?: { contest_number: number; draw_at?: string; synced_at?: string; numbers: number[]; special?: string | null } | null };
...
<template>
  <div class="app-shell">
    ...
    <main v-if="view === 'home'" class="main">
      ...
      <section class="product-grid"><article v-for="game in catalog" :key="game.id" class="product-card" :style="{ '--game-color': game.color }"><div class="product-card-head"><div class="product-brand"><div class="product-logo"><component :is="gameIcon(game)" :size="19" /></div><strong>{{ game.name }}</strong></div><button class="favorite" aria-label="Salvar jogo" @click.stop="notify('Jogo salvo nos favoritos.')"><Heart :size="15" /></button></div><div class="product-prize-label">Prêmio estimado do concurso {{ game.next_draw?.contest_number ?? '—' }}</div><strong class="product-prize">{{ estimatedPrize(game) }}</strong><div class="product-meta">Sorteio {{ drawDateTime(game.next_draw?.draw_at) }}</div><div v-if="game.latest_result?.numbers?.length" class="product-latest-result"><span>Último resultado · concurso <strong>{{ game.latest_result.contest_number }}</strong></span><div class="product-result-numbers"><b v-for="number in game.latest_result.numbers" :key="number">{{ String(number).padStart(2, '0') }}</b></div></div><div class="product-meta product-cutoff">Apostas abertas até o próximo concurso</div><button class="product-cta" @click.stop="chooseGame(game)">A partir de {{ money(game.price_cents) }} <ArrowRight :size="15" /></button></article></section>
