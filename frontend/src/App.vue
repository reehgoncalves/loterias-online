...
type CartTicket = { id: string; game: Game; draw_id?: number; numbers: number[]; amount_cents: number; kind?: 'game' | 'pool' | 'combo'; pool_id?: number; shares?: number; combo_id?: string; combo_title?: string };
...
type PoolCard = { id: number; slug: string; game: string; title: string; shares: string; price: number; draw_id?: number; color: string; totalShares: number; soldShares: number; numbers: number[]; drawLabel: string; description: string };
...
const demoCatalog: Game[] = [
  { id: 1, slug: 'mega-sena', name: 'Mega-Sena', short_name: 'MEGA', price_cents: 500, color: '#31b8b2', range_max: 60, numbers_required: 6, next_draw: { id: 1, contest_number: 2910, draw_at: '2026-08-08T20:00:00-03:00' } },
  { id: 2, slug: 'lotofacil', name: 'Lotofácil', short_name: 'FÁCIL', price_cents: 350, color: '#8c5be5', range_max: 25, numbers_required: 15, next_draw: { id: 2, contest_number: 3480, draw_at: '2026-08-07T20:00:00-03:00' } },
  { id: 3, slug: 'quina', name: 'Quina', short_name: 'QUINA', price_cents: 300, color: '#ef9151', range_max: 80, numbers_required: 5, next_draw: { id: 3, contest_number: 6820, draw_at: '2026-08-07T20:00:00-03:00' } },
  { id: 4, slug: 'timemania', name: 'Timemania', short_name: 'TIME', price_cents: 350, color: '#f05295', range_max: 80, numbers_required: 10, next_draw: { id: 4, contest_number: 2260, draw_at: '2026-08-09T20:00:00-03:00' } },
  { id: 5, slug: 'dia-de-sorte', name: 'Dia de Sorte', short_name: 'DIA', price_cents: 250, color: '#f1b833', range_max: 31, numbers_required: 7, next_draw: { id: 5, contest_number: 1080, draw_at: '2026-08-09T20:00:00-03:00' } },
  { id: 6, slug: 'dupla-sena', name: 'Dupla Sena', short_name: 'DUPLA', price_cents: 300, color: '#3d8de5', range_max: 50, numbers_required: 6, next_draw: { id: 6, contest_number: 2860, draw_at: '2026-08-08T20:00:00-03:00' } },
  { id: 7, slug: 'lotomania', name: 'Lotomania', short_name: 'LOTO', price_cents: 300, color: '#e061b7', number_min: 0, range_max: 99, numbers_required: 20, next_draw: { id: 7, contest_number: 2800, draw_at: '2026-08-08T20:00:00-03:00' } },
  { id: 8, slug: 'super-sete', name: 'Super Sete', short_name: '7', price_cents: 300, color: '#41a86d', number_min: 0, range_max: 9, numbers_required: 7, selection_mode: 'columns', special_options: { columns: 7 }, next_draw: { id: 8, contest_number: 730, draw_at: '2026-08-08T20:00:00-03:00' } },
];
...
const poolOffers: PoolCard[] = [
  { id: 1, slug: 'mega-sena', game: 'Mega-Sena', title: 'Milionário da Semana', shares: '87/100 cotas', price: 12.5, color: '#31b8b2', totalShares: 100, soldShares: 87, numbers: [4, 11, 19, 27, 42, 58], drawLabel: 'Próximo concurso · sábado, 20h', description: 'Cota demonstrativa com combinação selecionada e acompanhamento por concurso.' },
  { id: 2, slug: 'lotofacil', game: 'Lotofácil', title: 'Fácil Premiado', shares: '132/200 cotas', price: 7.9, color: '#8c5be5', totalShares: 200, soldShares: 132, numbers: [1, 3, 5, 7, 8, 10, 12, 14, 16, 18, 20, 21, 22, 24, 25], drawLabel: 'Próximo concurso · hoje, 20h', description: 'Cota demonstrativa com seleção ampla para você acompanhar de forma simples.' },
  { id: 3, slug: 'quina', game: 'Quina', title: 'Quina Turbo', shares: '42/80 cotas', price: 9.5, color: '#ef9151', totalShares: 80, soldShares: 42, numbers: [7, 18, 29, 44, 73], drawLabel: 'Próximo concurso · hoje, 20h', description: 'Cota demonstrativa com cinco números e status atualizado do bolão.' },
];
const promoSlides = [
...
function addComboToCart(combo: ComboOffer) {
  ...
  games.forEach((game, index) => cart.value.push({ id: `${combo.id}-${game.id}-${Date.now()}-${index}`, game, draw_id: game.next_draw?.id, numbers: localCoupon(game), amount_cents: game.price_cents, kind: 'combo', combo_id: combo.id, combo_title: combo.title }));
...
<template>
  <div class="app-shell">
    ...
    <main v-if="view === 'home'" class="main">
      ...
      <section class="product-grid"><article v-for="game in catalog" :key="game.id" class="product-card" :style="{ '--game-color': game.color }"><div class="product-card-head"><div class="product-brand"><div class="product-logo"><component :is="gameIcon(game)" :size="19" /></div><strong>{{ game.name }}</strong></div><button class="favorite" aria-label="Salvar jogo" @click.stop="notify('Jogo salvo nos favoritos.')"><Heart :size="15" /></button></div><div class="product-prize-label">Prêmio estimado do concurso {{ game.next_draw?.contest_number ?? '—' }}</div><strong class="product-prize">{{ estimatedPrize(game) }}</strong><div class="product-meta">Sorteio {{ shortDate(game.next_draw?.draw_at) }}</div><div class="product-meta product-cutoff">Apostas abertas até o próximo concurso</div><button class="product-cta" @click.stop="chooseGame(game)">Aposte por {{ money(game.price_cents) }} <ArrowRight :size="15" /></button></article></section>
