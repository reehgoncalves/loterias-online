...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $games = [
            ...
            ['slug'=>'lotofacil','name'=>'Lotofácil','short_name'=>'FÁCIL','color'=>'#8c5be5','price_cents'=>350,'numbers_required'=>15,'range_max'=>25,'payout_rules'=>['11'=>5,'12'=>10,'13'=>50,'14'=>250,'15'=>1000],'max_prize_cents'=>30000000],
            ['slug'=>'quina','name'=>'Quina','short_name'=>'QUINA','color'=>'#ef9151','price_cents'=>300,'numbers_required'=>5,'range_max'=>80,'payout_rules'=>['2'=>3,'3'=>15,'4'=>100,'5'=>1000],'max_prize_cents'=>20000000],
        ...
        foreach ($sample as $index => $fixture) {
            $bet = Bet::updateOrCreate(['idempotency_key'=>'seed-bet-'.$index], ['user_id'=>$customer->id,'lottery_game_id'=>$sampleGame->id,'draw_id'=>$sampleDraw->id,'numbers'=>$fixture['numbers'],'amount_cents'=>500,'potential_prize_cents'=>500000,'payout_cents'=>$fixture['payout_cents'],'status'=>$fixture['status'],'payment_status'=>'succeeded','paid_at'=>now()->subDays(2),'settled_at'=>now()->subDay(),'won_at'=>$fixture['status']==='won'?now()->subDay():null]);
            $payment = Payment::updateOrCreate(['idempotency_key'=>'seed-payment-'.$index], ['user_id'=>$customer->id,'bet_id'=>$bet->id,'provider'=>'stripe','provider_payment_id'=>'pi_demo_'.$index,'method'=>$index===0?'pix':'card','amount_cents'=>500,'currency'=>'brl','status'=>'succeeded','paid_at'=>now()->subDays(2),'raw_payload'=>['demo'=>true]]);
            LedgerEntry::updateOrCreate(['idempotency_key'=>'seed-ledger-payment-'.$index], ['user_id'=>$customer->id,'bet_id'=>$bet->id,'payment_id'=>$payment->id,'type'=>'payment_confirmed','amount_cents'=>500,'status'=>'posted','metadata'=>['demo_fixture'=>true]]);
