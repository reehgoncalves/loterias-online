...
class CatalogController extends Controller
{
    public function index(): JsonResponse {
        $games = LotteryGame::query()->where('active', true)->with(['draws'=>fn ($q) => $q->where('status','open')->orderBy('draw_at')->limit(1)])->get()->map(function (LotteryGame $game) {
            ...
            return ['id'=>$game->id,'slug'=>$game->slug,'name'=>$game->name,'short_name'=>$game->short_name,'price_cents'=>$game->price_cents,'color'=>$game->color,'range_max'=>$game->range_max,'numbers_required'=>$game->numbers_required,'next_draw'=>$next ? ['id'=>$next->id,'contest_number'=>$next->contest_number,'draw_at'=>$next->draw_at] : null];
        });
